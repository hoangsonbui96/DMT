...
class EqExportSheet extends AdminController implements FromView, WithTitle, WithColumnFormatting, WithEvents
{

    private $record;
    public function __construct($record,$requests,$request){
        $this->record = $record;
    ...
    public function view(): View\View
    {
        ...
        return $this->viewAdminLayout('eqEquipment-export', $this->data);
    }
    ...
    public function registerEvents(): array
    {
        ...
        $list = Equipment::query()->orderBy('id', 'desc');
        if(isset($search)){
            // $one = Equipment::query()->select('code','name','serial_number')->first();
            // if($one){
            //     $one = $one->toArray();
        ...
        return [
             AfterSheet::class => function(AfterSheet $event) use ($intTotalRow){
                ...
                $event->sheet->getDelegate()->getStyle('A2:M'.$intTotalRow)
