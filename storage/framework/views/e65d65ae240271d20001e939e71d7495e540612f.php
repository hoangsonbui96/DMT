<?php $__env->startSection('content'); ?>
    ...
    <section class="content">
                ...
                <form class="form-inline" method="get" id="timekeeping-search">
                    <div class="form-group pull-left margin-r-5 date" id="date">
                        <div class="input-group search date">
                            <input type="text" class="form-control" id="date-input" name="time" value="<?php echo e(isset(Request::all()['time']) ? Request::all()['time'] : Carbon\Carbon::now()->format('m/Y')); ?>" >
                    ...
                    <div class="form-group pull-right">
                        <a id= "exportExcel" class="btn btn-success" target="_blank" href="<?php echo e(route("admin.ExportTimeKeepingCompany")); ?>?date="><?php echo app('translator')->get('admin.export-excel'); ?></a>
                    </div>
                    <?php endif; ?>
                </form>
            </div>
        ...
        <div class="table-responsive table-timekeeping">
            <table class="table data-table" id="table_timekeeping">
                ...
                <tbody>
                    <tbody>
                        <?php $__currentLoopData = $allMonths; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $currentDate): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                            <tr date-item="<?php echo e($key); ?>" <?php echo e($currentDate['date'] == 'T7' ? 'class=weekend-7' : ''); ?> <?php echo e($currentDate['date'] == 'CN' ? 'class=weekend-cn' : ''); ?>>
                                <td  class="date-td"><?php echo e(\Carbon\Carbon::parse($key)->format('d/m/Y')); ?></td>
                                <td><?php echo e($currentDate['date']); ?></td>
                                <td colspan="2" class="th-modal hover-point" id="latecomers">
                                    <?php if(count($currentDate["arrUserActive"]) != (count($currentDate["arrUserActive"]) - count($currentDate["arrUserLateMonth"]) - count($currentDate["arrUserSoonMonth"]))): ?>
                                        <?php echo e(count($currentDate["arrUserLateMonth"])); ?>

                                    <?php endif; ?>
                                </td>
                                <td class="th-modal hover-point thead-th-custom" id="backSoon" rowspan="1">
                                    <?php if(count($currentDate["arrUserActive"]) != count($currentDate["arrUserActive"]) - count($currentDate["arrUserLateMonth"]) - count($currentDate["arrUserSoonMonth"])): ?>
                                        <?php echo e(count($currentDate["arrUserBackSoonMonth"])); ?>

                                    <?php endif; ?>
                                </td>
                                <td class="th-modal hover-point thead-th-custom"  id="notKeeping" rowspan="1">
                                    <?php if(count($currentDate["arrUserActive"]) != count($currentDate["arrUserActive"]) - count($currentDate["arrUserLateMonth"]) - count($currentDate["arrUserSoonMonth"])): ?>
                                        <?php echo e(count($currentDate["arrUserActive"]) - count($currentDate["arrUserLateMonth"]) - count($currentDate["arrUserSoonMonth"])); ?></td>
                                    <?php endif; ?>

                                <td class="th-modal hover-point thead-th-custom" id="tkKeeping" rowspan="1">
                                    <?php if(count($currentDate["arrUserActive"]) != (count($currentDate["arrUserActive"]) - count($currentDate["arrUserLateMonth"]) - count($currentDate["arrUserSoonMonth"]))): ?>
                                        <?php echo e(count($currentDate["arrUserLateMonth"]) + count($currentDate["arrUserSoonMonth"])); ?>

                                    <?php endif; ?>
                                </td>
                                <td class="th-modal hover-point thead-th-custom" id="tkHome" rowspan="1">
                                    <?php if(count($currentDate["arrUserActive"]) != (count($currentDate["arrUserActive"]) - count($currentDate["arrUserLateMonth"]) - count($currentDate["arrUserSoonMonth"]))): ?>
                                        <?php echo e(count($currentDate["arrUserCheckinAtHomeMonth"])); ?>

                                    <?php endif; ?>
                                </td>
                                <td class="th-modal hover-point thead-th-custom" id="tkCompany" rowspan="1">
                                    <?php if(count($currentDate["arrUserActive"]) != (count($currentDate["arrUserActive"]) - count($currentDate["arrUserLateMonth"]) - count($currentDate["arrUserSoonMonth"]))): ?>
                                        <?php echo e(count($currentDate["arrUserCheckinAtCpnMonth"])); ?>
    ...
    <script language="javascript">
        const data = <?php echo json_encode($allMonths, 15, 512) ?>;
        ...
        $(".th-modal").click(event => {
            ...
            if ($(self).text() != 0) {
                ...
                ajaxGetServerWithLoader("<?php echo e(route('admin.latecomers')); ?>" + '/' + id + '/' + dateItem, "GET", null, function (data) {
        ...
        $(function () {
            ...
            $("#exportExcel").attr("href", "<?php echo e(route("admin.ExportTimeKeepingCompany")); ?>?date=" + $("#date-input").val());
        })
    </script>
...
<?php echo $__env->make('admin.layouts.default.app', \Illuminate\Support\Arr::except(get_defined_vars(), ['__data', '__path']))->render(); ?><?php /**PATH D:\DMT\resources\views/admin/layouts/default/checkin/timekeepingCompany.blade.php ENDPATH**/ ?>