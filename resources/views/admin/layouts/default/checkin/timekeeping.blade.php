						...
						<thead class="thead-default">
							<tr>
								<th colspan="12" style="background: rgba(255, 225, 0, 0.5); text-align: left !important;">
									@lang('admin.Staffs_name'):&nbsp; {{ $userSelect->FullName }}
								</th>
							</tr>
							<tr>
								...
								<th>{{ number_format($timekeepings->lateHours/60, 2) }}</th>
{{--								<th rowspan="4"></th>--}}
							</tr>
							<tr>
								<th>@lang('admin.timekeeping.overtime')</th>
								<th colspan="2">{{ number_format($timekeepings->overKeeping/60, 2) }}</th>
								...
								<th>{{ number_format($timekeepings->soonHours/60, 2) }}</th>
							</tr>
{{--							<tr>--}}
					...
					<table class="table data-table" id="table_timekeeping">
						<thead class="thead-default">
							<tr>
								...
								<th class="thead-th-custom" rowspan="2" style="width:200px">@lang('admin.timekeeping.type')</th>
						...
						@foreach($timekeepings as $timekeeping)
							<tr {{ $timekeeping->weekday == 'T7' ? 'class=weekend-7' : '' }} {{ $timekeeping->weekday == 'CN' ? 'class=weekend-cn' : '' }} >
								<td id="tk-date">{{ \Carbon\Carbon::parse($timekeeping->Date)->format('d/m/Y') }}</td>
								<td>{{ $timekeeping->weekday }}</td>
								<td>{{ isset($timekeeping->TimeIn) ? \Carbon\Carbon::parse($timekeeping->TimeIn)->format("H:i:s") : '' }}</td>
								<td>{{ isset($timekeeping->TimeOut) ? \Carbon\Carbon::parse($timekeeping->TimeOut)->format("H:i:s") : '' }}</td>
								<td>
									@if($timekeeping->STimeOfDay && $timekeeping->ETimeOfDay != null)
										{{ $timekeeping->STimeOfDay}} - {{isset( $timekeeping->SBreakOfDay) ?  $timekeeping->SBreakOfDay : \Carbon\Carbon::parse(App\MasterData::where('DataValue', 'WT002')->first()->Name)->format("H:i:s") }} <br>
										{{ isset($timekeeping->EBreakOfDay)  ? $timekeeping->EBreakOfDay :  \Carbon\Carbon::parse(App\MasterData::where('DataValue', 'WT002')->first()->DataDescription)->format("H:i:s")}} - {{ $timekeeping->ETimeOfDay }}

									@endif

								</td>
								<td>{{ $timekeeping->late != "00:00:00" ? \Carbon\Carbon::parse($timekeeping->late)->format("H:i:s") : '' }}</td>
								<td>{{ $timekeeping->soon != "00:00:00" ? \Carbon\Carbon::parse($timekeeping->soon)->format("H:i:s") : '' }}</td>
								<td>{{  round($timekeeping->hours, 2, PHP_ROUND_HALF_UP) }}</td>
								<td>{{ isset($timekeeping->hoursTT) ? \Carbon\Carbon::parse($timekeeping->hoursTT)->format("H:i:s") : '' }}</td>
								...
								<td>
									@foreach($timekeeping->absence as $absence)
										...
										@endif
									@endforeach
									@if($timekeeping->calendarEvent)
										<a class="action-col view" style="text-decoration: none">Làm bù ({{ $timekeeping->calendarEvent->StartDate }})</a><br/>
									@endif
								</td>
								<td>
									@if (isset($timekeeping->type) && count($timekeeping->type) > 0)
										@foreach($timekeeping->type as $type)
											<span>{{ $type->Type }}</span><br>
										@endforeach
									@elseif ($timekeeping->id)
								...
								@if ($canEdit || $canDelete)
									<td class="text-center">
										@if ($canEdit)
											<span class="action-col update edit update-timekeeping" item-id="{{ $timekeeping->id }}" item-date="{{ $timekeeping->Date }}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span>
										@endif
										@if ($canDelete && $timekeeping->id)
					...
					<div>
						<div class="modal-dialog modal-lg ui-draggable width550">
							...
							<div class="modal-content drag">
								<form class="form-horizontal" action="{{ route('admin.importTimekeepingNew') }}" method="POST" enctype="multipart/form-data">
...
@section('js')
	<script type="text/javascript" async>
		...
		var ajaxUrl_detail = '{{ route('admin.detailTimekeepingNew') }}';
		...
		$(function () {
			...
			$('#btn-export-timekeeping').click(function (e) {
				...
				if($('#exportModal .modal-title').text() == 'Xuất dữ liệu vắng mặt'){
					ajaxServer('{{ route('admin.exportAbsenceTimekeepingNew') }}/'+timeExport+'/'+User, 'GET',null, function (data) {
						if (typeof data.errors !== 'undefined') {
							$('.loadajax').hide();
							showErrors(data.errors[0]);
						} else {
							window.location.href = '{{ route('admin.exportAbsenceTimekeepingNew') }}/'+timeExport+'/'+User;
					...
					ajaxServer('{{ route('admin.exportTimekeepingNew') }}/'+timeExport+'/'+User, 'GET',null, function (data) {
						if (typeof data.errors !== 'undefined') {
							$('.loadajax').hide();
							showErrors(data.errors[0]);
						} else {
							window.location.href = '{{ route('admin.exportTimekeepingNew') }}/'+timeExport+'/'+User;
							$('#exportModal').modal('hide');
							// $('#select-status option[value=1]').attr('selected','selected');
							// selectUser();
							$('.loadajax').hide();
		...
		$('#table_timekeeping tbody tr td a.view').click(function() {
			...
			ajaxGetServerWithLoader('{{route('admin.AbsenceTimekeepingNew')}}', 'POST', {
				date: date,
				UserID: UserID,
			}, function (data) {
				$('#popupModal').empty().html(data);
