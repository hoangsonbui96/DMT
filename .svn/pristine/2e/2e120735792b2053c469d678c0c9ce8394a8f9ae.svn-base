						...
						<tr class="even gradeC" data-id="">
                            ...
                            @if ($canEdit || $canDelete)
                                <td class="text-center">
                                    @if ($canEdit && ($canAppr || (!$canAppr && $item->Approved != 1 && $item->UID == \Illuminate\Support\Facades\Auth::user()->id)))
                                        <span class="action-col update edit update-one" item-id="{{ $item->id }}"><i class="fa fa-pencil-square-o" aria-hidden="true"></i></span>
                                    @endif
                                    @if ($canDelete && ($canAppr || (!$canAppr && $item->Approved != 1 && $item->UID == \Illuminate\Support\Facades\Auth::user()->id)))
